...
def extract_rich_features(view, stage, records, device, amp, batch_size, num_workers):
...
def load_or_extract(cache, view, stage, records, device, amp, batch_size, num_workers):
    path = Path(cache) / f"rich_{view}_{stage}.npz"
    if path.exists():
    ...
    data = extract_rich_features(view, stage, records, device, amp, batch_size, num_workers)
...
def main() -> int:
    ...
    print(f"Extracting rich features for {len(records)} {args.view} runs (frozen {BACKBONE.pretrained_name})…")
